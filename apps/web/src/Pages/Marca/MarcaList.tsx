  ...
  },
  {
    ...
    render: (record: any) => (
      <Tooltip label={'Editar ' + record.titulo} position="left">
        <Link to={`/marcas/actualizar/${record.id}`}>
...
export const MarcaList = () => {
  ...
  const onError = (data: any) => {
    showNotification({
      title: <Text fw="bold">Falha na busca das marcas</Text>,
      message: (
  ...
  const {
    ...
    isLoading,
    isError,
  } = useMarcasData({ onError, filter: { titulo }, page, perPage });
  ...
  const reset = () => {
    const form: any = document.getElementById('filterMarca');
    form.reset();
    setTitulo('');
  };

  const filterForm = (
    <form id="filterMarca" onSubmit={(e) => onFilterSubmit(e)}>
      <SimpleGrid
        cols={3}
        breakpoints={[
  ...
  return (
    <>
      <TitleBar title={'Marcas'} rightSection={rightSection} />
      <Paper shadow="sm">
